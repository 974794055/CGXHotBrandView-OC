...
- (void)layoutSubviews
{
    ...
    for (int i = 0; i<self.titleArray.count; i++) {
        ...
        CGFloat width = [str boundingRectWithSize:CGSizeMake(ScreenWidth, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName : [UIFont systemFontOfSize:14]} context:nil].size.width+20;
...
{
    if (self.currentInter == btn.tag-10000 ) {
        if (!self.isFirst) {
            return;
        }
    }
    ...
    self.isFirst = NO;
    self.currentInter = btn.tag-10000;
